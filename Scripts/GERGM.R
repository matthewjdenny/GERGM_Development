...
# TODO (James): 
# 1: statistics functions: MCMC diagnostics, likelihood value, deviance
...
mcmcmle <- function(formula.obj, num.draws, mc.num.iterations,
    ...
    #print(alphas)
    net2 <- res1$net #Fixing this to avoid global call 2/12/15: issue -- seems to call the same network each time
    #thin <- round(1/take.sample.every, 2)
    # calculate parameter estimates for theta
    # Cancelling out the mple (12/9)
    if(CUSTOM){
        theta.init <- list()
...
gergm <- function(object, directed = c(TRUE, FALSE), transform.data = NULL,
    ...
    if (is.null(transform.data) != TRUE) {
        ...
        for (i in 1:max.num.iterations) {
            ...
            # TODO: Use theta standard error from previous estimate: JW -> we need an initial hessian 
            # for the first run.
            theta.std.errors <- 1 / sqrt(abs(diag(theta.new$hessian)))
            
            
            # Stop if lambda and theta estimates converge
            # JW: Changing this criterion (April 2nd, 2015)
...
#             t.stat <- qt(1 - (tolerance / 2), 1000000)
#             bounds <- t.stat * theta.std.errors
#             bounds2 <- t.stat * gpar.std.errors
#             count <- rep(0,length(theta$par))
#             count2 <- rep(0, length(gpar$par))
#             for(i in 1:length(theta$par)){
#               count[i] <- abs(theta.new$par[i] - theta$par[i]) > bounds[i]
#             }
#             for(i in 1:length(gpar$par)){
#               count2[i] <- abs(gpar.new$par[i] - gpar$par[i]) > bounds2[i]
#             }
            #cat(count, count2)
            #if (max(abs(theta.new$par - theta$par)) < tolerance) {}
    ...
    if (is.null(transform.data) == TRUE) {
        ...
        theta.new <- mcmcmle(formula.obj = object, num.draws = nsim, 
                             ...
                             together = together, tolerance = tolerance)
        theta.std.errors <- 1 / sqrt(abs(diag(theta.new$hessian)))
        theta <- theta.new
